	...
	for (const auto& entry : actionTable) 
	{
        ...
        if (actions.size() > 1) 
        {
            ...
            if (hasShift && hasReduce) 
            {
                ...
                cout<<"Hence, This grammar cannot be parsed by LR(0) Parser"<<endl;
                break;
            }
...
{
    string input;
    cout<<"LR(0) Parser worked for the following Grammar: "<<endl;
    cout<<"E->E+T|T"<<endl;
    cout<<"T->T*F|F"<<endl;
    cout<<"F->id|(E)"<<endl;
    ...
    {
        cout << "Error: Invalid input string. Only '+', '*', '(', ')', 'id', and '$' are allowed." << endl;
        return 1;
    }
    ...
    while (true) 
    {
        if (i>=input.length()) 
        {
            cout<<"Error: Input not fully consumed. Parsing failed."<<endl;
            return 1;
        }
        if (stateStack.empty() || symbolStack.empty()) 
        {
            cout<<"Error: Stack underflow. Parsing failed."<<endl;
            return 1;
        }
        ...
        if (currentState<0 || currentState>11) 
        {
        ...
        {
            cout<<"Error: No action found for state "<<currentState<<" and symbol "<<currentInputSymbol<<endl;
            return 1;
        }
        ...
        if (hasAcceptAction(currentStateMap)) 
        {
